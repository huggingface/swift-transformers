...
class HubApiTests: XCTestCase {
    ...
    func testGetFileMetadata() async throws {
        do {
            let url = URL(
                string:
                    "https://huggingface.co/coreml-projects/Llama-2-7b-chat-coreml/resolve/main/config.json"
            )
    ...
    func testGetFileMetadataWithRevision() async throws {
        do {
            ...
            let url = URL(
                string:
                    "https://huggingface.co/julien-c/dummy-unknown/resolve/\(revision)/config.json"
            )
            let metadata = try await Hub.getFileMetadata(fileURL: url!)
