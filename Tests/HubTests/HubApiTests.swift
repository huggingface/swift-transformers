...
class HubApiTests: XCTestCase {
    ...
    func testMetadataCacheHit() async {
        do {
            let hub = HubApi()
            let testUrl = URL(string: "https://huggingface.co/coreml-projects/Llama-2-7b-chat-coreml/resolve/main/config.json")!
            let cacheKey = MetadataCacheKey(url: testUrl, hasAuth: hub.hfToken != nil)

            // Clear cache to ensure a clean state for this test
            ...
            XCTAssertEqual(
                response1.value(forHTTPHeaderField: "Etag"),
                response2.value(forHTTPHeaderField: "Etag")
            )
        } catch {
    ...
    func testMetadataCacheDifferentUrls() async {
        do {
            let hub = HubApi()
            let url1 = URL(string: "https://huggingface.co/coreml-projects/Llama-2-7b-chat-coreml/resolve/main/config.json")!
            let url2 = URL(string: "https://huggingface.co/coreml-projects/Llama-2-7b-chat-coreml/resolve/main/tokenizer.json")!
            let cacheKey1 = MetadataCacheKey(url: url1, hasAuth: hub.hfToken != nil)
            let cacheKey2 = MetadataCacheKey(url: url2, hasAuth: hub.hfToken != nil)
            ...
            let key1IsCached = await HubApi.metadataCache.contains(cacheKey1)
            let key2IsCached = await HubApi.metadataCache.contains(cacheKey2)
            XCTAssertTrue(key1IsCached, "First URL should be cached")
            XCTAssertTrue(key2IsCached, "Second URL should be cached")

            // Cache should contain 2 entries
            let finalCount = await HubApi.metadataCache.count
            XCTAssertEqual(finalCount, 2, "Cache should contain 2 entries")

            // Different URLs should have different responses
            XCTAssertNotEqual(response1.url, response2.url)
        } catch {
            XCTFail("Unexpected error: \(error)")
        }
    }

    func testMetadataCacheAuthDifferentiation() async {
        do {
            let testUrl = URL(string: "https://huggingface.co/coreml-projects/Llama-2-7b-chat-coreml/resolve/main/config.json")!
            ...
            // Create cache keys - one without auth, one with auth
            let cacheKeyNoAuth = MetadataCacheKey(url: testUrl, hasAuth: false)
            let cacheKeyWithAuth = MetadataCacheKey(url: testUrl, hasAuth: true)

            // Verify cache keys are different
            XCTAssertNotEqual(cacheKeyNoAuth, cacheKeyWithAuth, "Cache keys should differ by auth status")
            ...
            // Manually set the same response in cache with different auth states
            await HubApi.metadataCache.set(cacheKeyNoAuth, response: response)
            await HubApi.metadataCache.set(cacheKeyWithAuth, response: response)

            // Verify both entries exist in cache as separate entries
            let noAuthIsCached = await HubApi.metadataCache.contains(cacheKeyNoAuth)
            let withAuthIsCached = await HubApi.metadataCache.contains(cacheKeyWithAuth)
            XCTAssertTrue(noAuthIsCached, "No-auth entry should be cached")
            XCTAssertTrue(withAuthIsCached, "Auth entry should be cached separately")

            // Verify both entries exist in cache (2 separate cache entries)
            let cacheCount = await HubApi.metadataCache.count
            XCTAssertEqual(cacheCount, 2, "Cache should have 2 separate entries for different auth states")

            // Verify we can retrieve both entries
            let retrievedNoAuth = await HubApi.metadataCache.get(cacheKeyNoAuth)
            let retrievedWithAuth = await HubApi.metadataCache.get(cacheKeyWithAuth)
            XCTAssertNotNil(retrievedNoAuth, "Should retrieve no-auth entry")
            XCTAssertNotNil(retrievedWithAuth, "Should retrieve auth entry")
        } catch {
            XCTFail("Unexpected error: \(error)")
        }
    }

    func testGetFileMetadataUsesCache() async {
        do {
            let hub = HubApi()
            let testUrl = URL(string: "https://huggingface.co/coreml-projects/Llama-2-7b-chat-coreml/resolve/main/config.json")!
            let cacheKey = MetadataCacheKey(url: testUrl, hasAuth: hub.hfToken != nil)
            ...
            await HubApi.metadataCache.clear()
            let initiallyIsCached = await HubApi.metadataCache.contains(cacheKey)
            XCTAssertFalse(initiallyIsCached)

            // First call
            let metadata1 = try await hub.getFileMetadata(url: testUrl)
            XCTAssertNotNil(metadata1.etag)
            XCTAssertNotNil(metadata1.size)

            let isCached = await HubApi.metadataCache.contains(cacheKey)
            XCTAssertTrue(isCached)

            // Second call should use cache
            let metadata2 = try await hub.getFileMetadata(url: testUrl)
            XCTAssertEqual(metadata1.etag, metadata2.etag)
            XCTAssertEqual(metadata1.size, metadata2.size)
            XCTAssertEqual(metadata1.location, metadata2.location)
        } catch {
            XCTFail("Unexpected error: \(error)")
        }
    }

    func testMultipleRequestsReduceNetworkCalls() async {
        do {
            let hub = HubApi()
            let testUrl = URL(string: "https://huggingface.co/coreml-projects/Llama-2-7b-chat-coreml/resolve/main/config.json")!
            let cacheKey = MetadataCacheKey(url: testUrl, hasAuth: hub.hfToken != nil)
            ...
            // Only the first should hit the network, rest should be cached
            ...
            // All responses should have the same etag for the same file
            let etags = results.compactMap { $0.value(forHTTPHeaderField: "Etag") }
            let uniqueEtags = Set(etags)
            XCTAssertEqual(uniqueEtags.count, 1, "All cached responses should have the same etag")
        } catch {
            XCTFail("Unexpected error: \(error)")
        }
    }

    func testMetadataCacheExpiration() async {
        do {
            let hub = HubApi()
            let testUrl = URL(string: "https://huggingface.co/coreml-projects/Llama-2-7b-chat-coreml/resolve/main/config.json")!
            let cacheKey = MetadataCacheKey(url: testUrl, hasAuth: hub.hfToken != nil)
            ...
            await HubApi.metadataCache.set(cacheKey, response: response, ttl: .milliseconds(100))

            // Verify entry is in cache immediately
            let isCachedBeforeExpiry = await HubApi.metadataCache.contains(cacheKey)
            XCTAssertTrue(isCachedBeforeExpiry, "Entry should be in cache before expiration")

            // Wait for expiration (150ms to be safe)
            try await Task.sleep(for: .milliseconds(150))

            // Verify entry is no longer in cache after expiration
            let isCachedAfterExpiry = await HubApi.metadataCache.contains(cacheKey)
            XCTAssertFalse(isCachedAfterExpiry, "Entry should be expired and removed from cache")

            // Verify get() also returns nil for expired entry
            let cachedResponse = await HubApi.metadataCache.get(cacheKey)
            XCTAssertNil(cachedResponse, "get() should return nil for expired entry")
        } catch {
            XCTFail("Unexpected error: \(error)")
        }
    }
