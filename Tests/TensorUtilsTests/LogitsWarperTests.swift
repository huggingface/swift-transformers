...
final class LogitsWarperTests: XCTestCase {
    ...
    func testTopKLogitsWarper() {
        ...
        let result5 = TopKLogitsWarper(k: 4)(
            [0, 1, 2, 3, 4, 5],
            [2.0, 1.0, 3.0, -1.0, 123.0, 0.0]
        )
    ...
    func testRepetitionPenaltyWarper() {
        ...
        let result4 = RepetitionPenaltyWarper(penalty: 1.11)(
            [2, 3, 4],
            [0.5029, 0.8694, 0.4765, 0.9967, 0.4190, 0.9158]
        )
        XCTAssertEqual(result4.indices, [2, 3, 4])
        XCTAssertEqual(
            result4.logits,
            [0.5029, 0.8694, 0.4293, 0.8980, 0.3775, 0.9158],
            accuracy: 1e-4
        )

        let result5 = RepetitionPenaltyWarper(penalty: 0.9)(
            [0, 1, 2],
            [-0.7433, -0.4738, -0.2966]
        )
        XCTAssertEqual(result5.indices, [0, 1, 2])
        XCTAssertEqual(result5.logits, [-0.6690, -0.4264, -0.2669], accuracy: 1e-4)

        let result6 = RepetitionPenaltyWarper(penalty: 1.125)(
            [3, 1, 2],
            [0.1674, 0.6431, 0.6780, 0.2755]
        )
